...
public class Parser {
	...
	public Parser(){
		...
		int rows= A.size();
		int colmns=A.get(0).size();
		Double[][] Anew= new Double[rows][colmns];
		
		for (int i=0;i<rows;i++) {
			for (int j=0;j<colmns;j++) {
				Anew[i][j]=A.get(i).get(j);
				
			}
		}
		//Converting the problem to its dual
		new DualConverter(Anew,rows,colmns,b,c,Eqin,MinMax);
